...
((STREAM_NumberPairs_out *) arbiter_outevent)->cases.NumberPair.m = rp;
         ...
         shm_monitor_buffer_write_finish(monitor_buffer);
        	shm_arbiter_buffer_drop(BUFFER_P0, 1); ARBITER_DROPPED_ = true;
	shm_arbiter_buffer_drop(BUFFER_P1, 1); ARBITER_DROPPED_ = true;
...
is_selection_successful = bg_get_first_n(&BG_Ps, 2, &chosen_streams);
    ...
    if (is_selection_successful) {
        int expected_matches_ = 2;
        int current_matches_ = 0;
        int index_F = 0;int index_S = 1;
        while(index_F < BG_Ps_size && current_matches_ < expected_matches_){

index_S = 0;
while(index_S < BG_Ps_size && current_matches_ < expected_matches_){
if(index_S == index_F){ index_S++; continue;}

shm_stream *F = chosen_streams[index_F]->stream;
shm_arbiter_buffer *BUFFER_F = chosen_streams[index_F]->buffer;
STREAM_Primes_ARGS *stream_args_F = (STREAM_Primes_ARGS *)chosen_streams[index_F]->args;
char* e1_F; size_t i1_F; char* e2_F; size_t i2_F;
int count_F = shm_arbiter_buffer_peek(BUFFER_F, 1, (void**)&e1_F, &i1_F, (void**)&e2_F, &i2_F);
shm_stream *S = chosen_streams[index_S]->stream;
shm_arbiter_buffer *BUFFER_S = chosen_streams[index_S]->buffer;
STREAM_Primes_ARGS *stream_args_S = (STREAM_Primes_ARGS *)chosen_streams[index_S]->args;
...
stream_args_F->pos+= n;
             ...
             if (F == EV_SOURCE_P_0) {
                 ...
                 right_drops += n;
                	...
                	shm_arbiter_buffer_drop(BUFFER_F, 1); ARBITER_DROPPED_ = true;
